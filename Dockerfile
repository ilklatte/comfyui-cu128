...
ARG BASE_IMAGE=runpod/comfyui:1.3.2-comfyuiv0.30.0-cuda12.8
FROM ${BASE_IMAGE}
...
# The official image copies this baked tree into /workspace/runpod-slim on
# first boot. Keep every image-managed node here, never in /workspace itself.
...
RUN --mount=type=cache,target=/root/.cache/pip \
    ...
    python3 -m pip uninstall -y cupy cupy-wheel cupy-cuda11x cupy-cuda13x 2>/dev/null || true; \
    ...
    printf '\nCUSTOM_IMAGE=coohh88/runpod-comfyui-cu128:cu128\n' >> /opt/comfyui-baked/.runpod-bundle-version; \
