...
Connect `ilklatte/comfyui-cu128` to CircleCI and add these project environment
variables:
...
matches `vN` is pushed. For example, pushing `v2` publishes both `cu128-v2`
and `cu128-latest`. The Docker repository is read from `pins.json`; no
